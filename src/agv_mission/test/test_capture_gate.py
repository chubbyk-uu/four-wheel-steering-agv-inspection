...
@pytest.fixture
def archive():
    writer=ArchiveWriter(flush_interval_s=.01)
    yield writer
    assert writer.close()
    assert not writer.errors,writer.last_error
...
def test_explicit_initial_disable_and_pending_enable_close_order(tmp_path,archive):
    n=Node();g=CaptureGate(n,tmp_path,True,archive=archive)
...
def test_failed_handshake_does_not_claim_active_capture(tmp_path,archive):
    n=Node();g=CaptureGate(n,tmp_path,True,archive=archive)
    g.request(True,0)
...
def test_close_records_cancellation_reason_and_exception_latches_failure(tmp_path,archive):
    n=Node();g=CaptureGate(n,tmp_path,True,archive=archive)
...
def test_camera_heartbeat_distinguishes_pause_from_disabled_and_stale_data(tmp_path,monkeypatch,archive):
    import json
...
def test_missing_or_replayed_camera_heartbeat_times_out(tmp_path,monkeypatch,archive):
    import json
    from agv_mission import capture
    clock=[10.];monkeypatch.setattr(capture.time,'monotonic',lambda:clock[0])
    n=Node();g=CaptureGate(n,tmp_path,True,archive=archive)
...
def test_state_before_first_ros_clock_is_not_a_camera_fault(tmp_path,archive):
    import json
    n=Node();n.get_clock=lambda:SimpleNamespace(now=lambda:SimpleNamespace(nanoseconds=0))
    g=CaptureGate(n,tmp_path,True,archive=archive)
    g.on_state(SimpleNamespace(data=json.dumps(dict(time_s=12.,enabled=False))))
    assert not g.error and g.heartbeat is None


def test_scan_interruptions_fault_instead_of_silent_partial_success(tmp_path,archive):
    import json
    n=Node();g=CaptureGate(n,tmp_path,True,archive=archive)
...
def test_failed_close_is_not_retried_or_reported_as_archived(tmp_path,archive):
    n=Node();g=CaptureGate(n,tmp_path,True,archive=archive)
