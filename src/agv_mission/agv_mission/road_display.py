...
def build_road_display(manifest,output,max_pixels=1024):
    ...
    output.mkdir(parents=True,exist_ok=False);xmin,xmax,ymin,ymax=scene['inspection_bounds_xy_m']
    ...
    return dict(mesh=(output/'road.obj').resolve().as_uri(),frame=scene['frame'],bounds=[xmin,xmax,ymin,ymax],triangles=2*count,texture_pixels=texture_pixels,rgba_mipmap_budget_bytes=texture_pixels*4*4//3)
