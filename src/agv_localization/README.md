...
`ekf_global`的`process_noise_covariance`中z项曾为0.02，而x/y是5e-05。0.02正是`ekf_local`给**每一个**位置轴的值——也就是`ekf_global`的起点，水平轴被收紧时z被漏下了。0.02 m²/s意味着z一秒能自行漂0.141 m；车体受地面和0.05 m悬挂行程约束，比x/y**更**不自由，不该是它们的400倍。
