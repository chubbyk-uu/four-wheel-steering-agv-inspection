...
`ekf_global`的`process_noise_covariance`中z项曾为0.02，而x/y是5e-05。0.02正是`ekf_local`给**每一个**位置轴的值——也就是`ekf_global`的起点，水平轴被收紧时z被漏下了。0.02 m²/s意味着z一秒能自行漂0.141 m；车体受地面和±0.05 m悬挂行程约束（静载实测关节停在+1.4~+1.8 mm），比x/y**更**不自由，不该是它们的400倍。

判据不是"覆盖审计过不过"，而是**滤波器自洽性**：报告σ_z为52.2 mm而真实误差sd只有21.9 mm，`err/σ`标准差0.426（自洽应为1.0）。同种子同轨迹的A/B（`validate_localization.py --profile normal`，输入GNSS垂直RMSE 29.3/29.2 mm相同）：融合垂直RMSE 23.1→12.3 mm、位置最大误差80.1→37.5 mm，水平9.1→9.0 mm与偏航0.254→0.255°不变。**真实误差与报告σ同向下降**，不是单方面压协方差。改为1e-3，保留相对水平20倍的裕度给路面起伏——那是车体法向接触约束观测不到的。
