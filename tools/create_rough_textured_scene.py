...
def main():
    p=argparse.ArgumentParser();p.add_argument('--source',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--peak-mm',type=float,default=3);a=p.parse_args()
    if not 0<a.peak_mm<=3:raise ValueError('peak must be in (0,3] mm')
