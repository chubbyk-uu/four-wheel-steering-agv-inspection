...
def check(manifest):
    ...
    rectangles=[];triangles=0;collision=0;display=0;display_pixels=0;collision_methods={}
    for asset in scene['assets']:
        ...
        # An asset may show GZ a lighter mesh than the one OptiX images, so
        # these are two counts. Reporting one would understate the split.
        display+=asset.get('display_mesh',{}).get('triangles',asset['triangles'])
    ...
    return dict(passed=True,scope='static asset integrity and geometry/texture contracts; not motion or rendered acceptance',
                ...
                display_partitions=len(rectangles),display_pixels=display_pixels,optix_triangles=triangles,gz_display_triangles=display,collision_triangles=collision,collision_methods=collision_methods,
