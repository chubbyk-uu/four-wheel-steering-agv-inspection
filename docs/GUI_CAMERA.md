...
有GUI时，运动控制器会等到CameraTracking插件连续稳定3秒后才启动；启动错开、GUI失败和D3D12重试等待期间车辆不会运动。默认跟车模式同时验证跟随目标与看向目标均为AGV；`follow_camera:=false`仍验证GUI已经就绪，但不改变自由相机。GUI就绪后退出会立即结束整个仿真，不会按启动故障重试。D3D12＋RViz默认错开6秒并允许启动阶段最多重试2次；原生Linux不启用该重试。就绪探针超时由同一份重试次数和间隔自动计算，包含每次GUI启动及传输余量；超过探针300秒能力的参数组合会在启动时拒绝。初始错开不计入该超时，因为探针与首个GUI在错开结束后同时启动。无头运行不经过GUI门控。
