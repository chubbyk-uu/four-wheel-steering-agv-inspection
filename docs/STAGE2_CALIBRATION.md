...
结果：[校正验证及持续测试](../results/stage2_linescan_calibration.json)，[本次网格环境标定文件](../results/linescan_calibration_grid.json)，[前后对比图](linescan_calibration_comparison.png)。保存的标定文件仅适用于记录的仿真安装与光照条件；来源图像路径及 SHA256 保留于文件中，原始大图位于 `/tmp/agv_measured_calibration_v2`，可按上述命令重新采集。
