...
聚合结果见[四轨采集报告](../results/stage3_rectangle_capture.json)，修订前后比较见[扫描稳定性](issues/SCAN_STABILITY.md)。ROS原图与磁盘像素逐块SHA256校验；每块行号跨度与实际行数一致；采集区域由独立真值评价纵向端点，要求同一个未中断传感器段跨过每轨整个ROI。轨迹间距1 m、幅宽1.5 m。未把少量稀疏标签当成逐像素覆盖证明，未做往返图像特征拼接，也未宣称暂停/补扫已完成。
