...
当前采用[单轮物理编码器](ENCODER_MODEL.md)：默认左前轮轴角、8000计数/圈和64/149固定乘除，标定距离不是地面真值。`projected_encoder`保留为兼容键：新配置有`wheel_encoder`时，它只选择四轮vx/vy/wz拟合的运动门限，不再选择触发源。旧归档未声明`wheel_encoder`时仍保留四轮车体X投影解释，禁止混淆两类元数据。

四轮拟合残差、横向速度、偏航速度和速度上限仍做采集保护；BRAKE有效运动继续采样。新单轮编码器在段内发生转向轴向正反分支切换时明确拒绝并关闭该段，不能把旧四轮投影的分支恢复验收直接算作新模型通过。
...
实际轮径仅用于轮胎接触/视觉，不传给控制与里程计补偿。后续拼接使用固定高度/倾角和每图首行水平位置/航向约束，不能借逐行成像真值消除轮径误差。
...
任务采集轮速门限取平台物理轮速上限加0.02 m/s数值容差，不使用本道名义速度作为截断门限。2026-09-12起车辆最高速度（`max_speed`，15 km/h）与额定采集速度（`rated_scan_speed`，10 km/h）分开：门限取前者＝4.1867 m/s，规划器按后者拒绝过快的扫描请求。此前二者是同一个值，额定速度因此没有留给闭环的余量。意外分段（含`unsupported_scan_motion`）须令任务FAULT，禁止残缺图像静默完成。故障后关闭握手若无法确认归档，不反复重试，不冒充已归档；面板区分传感器已关闭与归档未确认，须重启采集实例后继续。
