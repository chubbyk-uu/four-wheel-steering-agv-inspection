...
当前4096像素/1.5 m、默认4096行/块，可配置纵向行数；普通暂停保留缓存；结束/取消/故障时默认丢弃不足1000行尾图并记录缺口，达到阈值才保存实际行数。同轨沿程连续无重叠，重叠来自不同轨迹。现有归档每图标签参考末行，保留first及少量中间标签，不能套用面阵中心位姿。新条带方案采用首行估计位置/航向软约束，标签接口待版本化实现，不能直接重解释旧字段。
...
当前顺序：暗场/平场及横向光学校正→同道按行直接连接→首行水平位置/航向软约束与跨道特征联合优化连续条带→接缝检查/融合→统一亮度编码的查看版TIFF。采用固定标定高度和俯仰/横滚，不以动态导航逐行投影作为起点。不得把同轨无重叠文件当独立面阵图任意平移；不能逐块自动拉伸形成亮度接缝。保留线性数据、无效与饱和信息，长路段采用分块处理/BigTIFF预算。

2026-09-17确认：下一步采用严格每图一个首行估计标签、整体尺度＋稀疏连续映射的全条带联合优化。先做39/41 cm实际轮径（名义40 cm）短程双道对照，验收标线形状、接口错位与整体/局部长度，再扩展100 m。100×10 m约74.6亿像素，使用分块金字塔BigTIFF和按需读取查看器。算法、观察标准和预算见[全局条带设计](SURFACE_HEADING_STRIP_DESIGN.md)，均尚待实现，不改变下述已实现工具的能力范围。

## 校正后直接连续条带（当前基线，已实现）

```bash
python3 tools/concatenate_corrected_strips.py \
  --mission /path/to/mission \
  --profile results/camera_20mm_measured_profile.json \
  --output /path/to/new_strips --reverse-preview-tracks 1
```

按实际反向轨迹ID选择预览旋转参数；完整PNG始终保留采集顺序。工具验证校正适用性、行号连续及输出与校正块逐像素一致，不使用位置/姿态，不填补缺口。短程两道结果为4096×10422和4096×10340，图块边界不增加重叠；并排预览不是地理对齐。见[结果](../results/corrected_continuous_pair.json)。随机振动仅为待实现试验，详见[当前设计](SURFACE_HEADING_STRIP_DESIGN.md)。

## 短程双道重投影诊断（已实现，仅研究对照）

以下工具默认仍为dynamic，这是历史诊断CLI默认值，不是当前真机处理主线。
