# Mesa D3D12命令签名缓存泄漏（项目私有修复已部署）

**状态（2026-09-14）**：根因已确认，私有源码修正版已接入项目启动，支持切回系统库；修复后同一WSL实例六次100×10 m全区任务通过，原内存增长阻断已关闭。见[完整验收](../../results/section6_closure.json)和[部署/恢复指南](../MESA_SETUP.md)。系统库未覆盖；上游状态以最近已核验记录为准，本文不宣称已合入上游。

下文按排查顺序保留因果实验、临时库修改和源码部署记录；其中“正式修复待部署/长任务尚未完成”仅指当时阶段。当前结论不能由早期短测单独替代。
...
当时根因确认后仍需源码部署与整车长测；这些后续步骤现已完成，入口见页首。
