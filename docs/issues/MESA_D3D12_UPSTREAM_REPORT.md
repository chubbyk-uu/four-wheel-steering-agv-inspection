...
A subsequent source build of Ubuntu 25.2.8 with the one-line fix has completed into a private prefix, with an opt-in project launcher and system fallback. It has now passed the standalone reproducer: over draws 6001–22001, original indirect RSS rose from 195564 to 242772 KiB (2.950333 KiB/draw), corrected-source indirect RSS stayed at 119772 KiB, and original direct RSS stayed at 175824 KiB. Each run completed 24000 draws without GL errors, passed foreground/background checks, and produced the same full-frame checksum. Actual loaded libraries were verified. See [source-build provenance](../../results/mesa_d3d12_source_build.json) and [source runtime results](../../results/mesa_source_runtime_repro.json). The application's GUI and long capture tests have not been repeated with this build. Earlier tables below retain the original binary-copy experiment and must not be confused with this newer source-built result.
