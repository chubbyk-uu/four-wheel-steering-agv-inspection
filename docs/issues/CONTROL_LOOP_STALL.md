...
**2026-09-14复发一次**：第6节验收连跑中，一次GUI启动在`D3D12: Removing Device`后`QT_OPENGL`失败，任务未开始；静置60 s重试通过（n=1）。该次启动距上一个13分钟全区GPU重载任务结束仅20秒，**正是上面记下的那个混杂条件**。拆分以来约26次带GUI启动出现这1次，与改前2/12的点估计（置信区间约2%–48%）仍然分不开，所以既不能说拆分失效，也不能说它有效。当时环境为私有源码版Mesa（仅D3D12、无llvmpipe），但**这不是成因**：09-11那次同样的`QT_OPENGL`发生在系统Mesa下，`drisw`当时是可用的。下一步最省的实验不是继续盲跑，而是直接检验那个混杂条件——"重载任务结束即刻启动"对"冷却后启动"，把冷却时间变成受控变量。见[第6节收尾](../../results/section6_closure.json)。
