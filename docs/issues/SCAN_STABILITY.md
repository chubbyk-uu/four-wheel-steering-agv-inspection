...
### 2026-09-12：生产PASS发生率已量化为0

`tools/audit_scan_slip.py`用两个互相独立的判据扫描归档，两者都不依赖对方成立：

- **位姿标签**同时带`encoder_distance_m`与`camera_position_world_m`，逐区间算"沿扫描轴地面位移/编码器位移"。必须用**沿轴地面距离**而非三维距离——停车时悬挂卸载单独就有几毫米垂直位移，会把停滞读成前进。
- **像素**直接给答案：相邻扫描行相距0.3662 mm，在真实纹理上必然有可测差异，成像同一片地面则没有。判据是相邻行平均绝对差，阈值取**本轮自身分布**的中位数×0.4，不是拍脑袋的常数。

2026-09-12补：像素判据此前靠`glob('block_*.pgm')`自己找图，归档目录被清理或搬走时它就**无图可测**，而“没发现重复行”照样使`passed`为真。现改为按块配对（同词干的`.json`/`.pgm`，并核对`block_id`/`segment_id`/`rows`/首末`global_line`），**缺图是失败而不是未测**，且只审本任务的块而非整个可能被共用的传感器会话目录；`passed`同时决定退出码，此前退出码恒为0。

在额定10 km/h的完整100×10 m任务上实测（710块、2,879,741行、2822个标签区间，[结果](../../results/scan_slip_10kmh.json)）：

| 判据 | 结果 |
|---|---|
| 地面/编码器 中位数 | **0.999995**（5 ppm） |
| 地面/编码器 最小值 | 0.9817（暂停恢复的加速段，152 mm窗口欠3 mm） |
| 低于0.95的区间 | **0 / 2822** |
| 相邻行差中位数 | 9.236 |
| 判据阈值 | 3.694 |
| 全程实测最小相邻行差 | **5.654**（仍是阈值的1.5倍） |
| 含重复行的图 | **0 / 710** |
| 完全相同的相邻行 | **0** |

对照：原始故障的比值是**0.12**，且重复行差为0。检测器本身有回归证明能抓到它（`test_scan_slip.py`：23行、8.057 mm编码器对0.968 mm地面，算出0.115被判出；而加速时约1%的轮胎滑移不被误判）。

**仍未做的**：像素判据只证明没有**重复**纹理，没有评价清晰度、对比度或几何畸变；那属于光学标定，不在本项内。另外比值判据用的是仿真真值相机位置，**实车上跑不了**——估计位姿的厘米级噪声盖不住毫米级停滞；可迁移的是像素判据，它只需要归档图像。

根因侧的结构性保障另有两层：300 N·m保持力矩（本文下节）消除了停转轮被动滚动；2026-09-12起采集只在对轮完成、控制器进入DRIVE之后才开启（见[覆盖缺口](COVERAGE_TAIL_GAP.md)），大角度对轮不再发生在快门已开的时候。
