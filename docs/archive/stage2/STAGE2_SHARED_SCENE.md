...
报告 `results/stage2_shared_scene.json`；预览 `docs/images/shared_scene_gz.png`。测试数据 `/tmp/agv_shared_scene_v2`，交叉验证原始记录 `/tmp/agv_shared_validation_final`，AGV 启动日志 `/tmp/agv_shared_agv_sim.log`，包测试 `/tmp/agv_shared_tests_final.log`。
