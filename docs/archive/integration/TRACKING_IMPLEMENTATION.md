...
本文件记录单段阶段基线；后续矩形执行与采集见[矩形采集](../../RECTANGLE_CAPTURE.md)。该阶段交付独立单段跟踪器，使用融合定位执行前进、横移、后退和原地旋转。在该阶段验收中未接矩形任务或相机。本轮整车平移限速0.5 m/s，不代表10 km/h闭环验收，也没有开启线阵采图。
...
![时间位置参考、速度与独立物理姿态](../../images/tracking_time_profiles.png)
